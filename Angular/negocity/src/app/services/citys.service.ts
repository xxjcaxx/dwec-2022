...
export class CitysService {
  ...
  getCities():Observable<City[]>{
    const obsCities = this.http.get<{[key: string]: City}>(this.url+".json")
    .pipe(
  ...
  deleteCity(id:string){
     this.http.delete<City>(`${this.url}/${id}.json`)
    .pipe(mergeMap(() => this.getCities())).subscribe(()=>{});
  }
  ...
  public createRandomCity() {
    let newCity = {
      ...
      this.http.post<City>(this.url + '.json', JSON.stringify(newCity))
      .pipe(mergeMap(() => this.getCities())).subscribe(()=>{});
